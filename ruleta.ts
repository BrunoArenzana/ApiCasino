...
export class Ruleta {
    ...
    private ultimoColor: string | null = null;
    ...
    private mostrarMenu() {
        ...
        console.log('4. Salir');
        console.log(this.MAGENTA + `Saldo actual: $${this.saldo}` + this.RESET);
    }
    ...
    private agregarApuesta() {
        ...
        if (monto <= 0 || monto > this.saldo) {
            ...
            console.log('Monto inválido. Debe ser un número positivo y no mayor a su saldo');
            return;
        }
    ...
    private verificarApuestas() {
        ...
        this.apuestasActuales.forEach(apuesta => {
            ...
            //use interpolacion ternaria, comparamos el valor del atributo con una condicion, si no, es la otra opcion, lo vimos con marcelo 1 clase y luego
            //en tutorias con Rafa!
            console.log(
                `$${apuesta.monto} a ${apuesta.opcion} (${apuesta.tipo}): ` +
                `${gano ? 'GANADA' : 'PERDIDA'} ` +
                `${gananciaNeta >= 0 ? '+' : ''}$${gananciaNeta}`
            );
        });
